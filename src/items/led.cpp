...
void LED::setBrightness(double brightness) {
	...
	auto parseResult = domDocument.setContent(BreadboardSvg.value(m_filename));
	if (!parseResult) {
		return;
	}
...
{
	QDomDocument domDocument;
	auto parseResult = domDocument.setContent(viewLayerID == ViewLayer::Breadboard ? BreadboardSvg.value(m_filename) : IconSvg.value(m_filename));
	if (!parseResult) {
		return "";
	}
