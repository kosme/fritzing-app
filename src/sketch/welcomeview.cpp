...
QString cleanData(const QString & data) {
	...
	while (pos < data.size()) {
		...
		bool parseResult = doc.setContent(listItem, &errorMsg, &errorLine, &errorColumn);
		#endif
		if (parseResult) {
			listItems << listItem;
		} else {
			DebugDialog::debug(QString("Error reading data %1 %2 %3")
			#if QT_VERSION >= QT_VERSION_CHECK(6, 5, 0)
...
void WelcomeView::gotBlogSnippet(QNetworkReply * networkReply) {
	...
	if (!goodBlog) {
		...
		auto parseResult = doc.setContent(placeHolder);
		if (parseResult) {
